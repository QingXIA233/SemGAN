...
def get_outputs(inputs, network="tensorflow"):
    ...
    with tf.variable_scope("Model") as scope:
        ...
        encoded_input_a = encoder(stack_input_a, "enc_A")
        encoded_input_b = encoder(stack_input_b, "enc_B")

        prob_real_a_is_real = current_discriminator(stack_input_a, "d_A")
        prob_real_b_is_real = current_discriminator(stack_input_b, "d_B")

        fake_images_b = current_generator_img(encoded_input_a, name="g_A")
        fake_images_a = current_generator_img(encoded_input_b, name="g_B")

        fake_oh_labels_b = current_generator_map(encoded_input_a, name="g_A_c")
        fake_oh_labels_a = current_generator_map(encoded_input_b, name="g_B_c")
        ...
        encoded_fake_a = encoder(stack_fake_a, "enc_A")
        encoded_fake_b = encoder(stack_fake_b, "enc_B") 

        fake_labels_a = inv_one_hot.back_img(fake_oh_labels_a, palette)
        fake_labels_b = inv_one_hot.back_img(fake_oh_labels_b, palette)  

        cycle_images_a = current_generator_img(encoded_fake_b, "g_B")
        cycle_images_b = current_generator_img(encoded_fake_a, "g_A")

        cycle_oh_labels_b = current_generator_map(encoded_fake_a, name="g_A_c")
        cycle_oh_labels_a = current_generator_map(encoded_fake_b, name="g_B_c")
...
def encoder(input_enc, name="encoder"):
    ...
    with tf.variable_scope(name):
        ...
        o_c1 = layers.general_conv2d(
            pad_input, tf.constant(True, dtype=bool), nef, f, f, 1, 1, 0.02, name="c1")
        #o_c1.shape = (1, 256, 256, 64)

        o_c2 = layers.general_conv2d(
            o_c1, tf.constant(True, dtype=bool), nef * 2, ks, ks, 2, 2, 0.02, "SAME", "c2")
        #o_c2.shape = (1, 128, 128, 128)

        o_c3 = layers.general_conv2d(
            o_c2, tf.constant(True, dtype=bool), nef * 4, ks, ks, 2, 2, 0.02, "SAME", "c3")
...
def decoder_img(input_dec, name="img_generator"):
    ...
    with tf.variable_scope(name):
        ...
        o_c4 = layers.general_conv2d(o_d2, tf.constant(False, dtype=bool), IMG_CHANNELS, f, f, 1, 1,
                                     0.02, "SAME", "c4", do_relu=False)
        #o_c1.shape = (1, 256, 256, 3)

        out_dec_img = tf.nn.tanh(o_c4, "t1")
...
def decoder_map(input_dec, name="map_generator"):
    ...
    with tf.variable_scope(name):
        ...
        o_d1_end = layers.general_conv2d(o_d1, tf.constant(True, dtype=bool), nef * 2, ks, ks, 1, 1, padding='same', name='d1')
        ...
        o_d2_end = layers.general_conv2d(o_d2, tf.constant(True, dtype=bool), nef, ks, ks, 1, 1, 0.02, padding='same', name='d2')
        #o_d2_end.shape = (1, 256, 256, 64)

        o_c4 = layers.general_conv2d(o_d2_end, tf.constant(False, dtype=bool), N_CLASSES, f, f, 1, 1,
                                     0.02, padding='SAME', name="c4", do_relu=False)
        #o_c4.shape = (1, 256, 256, 19)

        '''# Reshape o_c4 to (1*256*256, 19) to use softmax activation
        new_o_c4 = tf.reshape(o_c4, [1*256*256, 19])

        #Use softmax activation
        act_o_c4 = tf.keras.activations.softmax(new_o_c4)

        # Reshape act_o_c4 back to (1, 256, 256, 19)
        back_o_c4 = tf.reshape(act_o_c4, [1, 256, 256, 19])

        out_dec_map = back_o_c4'''
        out_dec_map = softmax(o_c4)
