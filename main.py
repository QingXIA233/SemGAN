...
class SemGAN:
    ...
    def __init__(self, pool_size, lambda_cyc_a, lambda_cyc_b,
        ...
        self._img_dataset_name = img_dataset_name
        self._label_dataset_name = label_dataset_name
        ...
        self.fake_labels_A = np.zeros(
            (self._pool_size, 1, model.IMG_HEIGHT, model.IMG_WIDTH,
             model.IMG_CHANNELS)
        )
        self.fake_labels_B = np.zeros(
            (self._pool_size, 1, model.IMG_HEIGHT, model.IMG_WIDTH,
             model.IMG_CHANNELS)
        )
    ...
    def model_setup(self):
        ...
        self.fake_pool_A_label = tf.placeholder(
            tf.float32, [
                None,
                model.IMG_WIDTH,
                model.IMG_HEIGHT,
                model.IMG_CHANNELS
            ], name="fake_pool_A_label")
        self.fake_pool_B_label = tf.placeholder(
            tf.float32, [
                None,
                model.IMG_WIDTH,
                model.IMG_HEIGHT,
                model.IMG_CHANNELS
            ], name="fake_pool_B_label")
        ...
        inputs = {
            ...
            'labels_a': self.label_a,
            'labels_b': self.label_b,
            'fake_pool_a': self.fake_pool_A,
            'fake_pool_b': self.fake_pool_B,
            'fake_pool_a_label': self.fake_pool_A_label,
            'fake_pool_b_label': self.fake_pool_B_label
        }
        ...
        self.oh_labels_a = outputs['oh_labels_a']
        self.oh_labels_b = outputs['oh_labels_b']

        self.fake_images_a = outputs['fake_images_a']
        self.fake_images_b = outputs['fake_images_b']

        self.fake_labels_a = outputs['fake_labels_a']
        self.fake_labels_b = outputs['fake_labels_b']

        self.fake_oh_labels_a = outputs['fake_oh_labels_a']
        self.fake_oh_labels_b = outputs['fake_oh_labels_b']
        ...
        self.cycle_oh_labels_a = outputs['cycle_oh_labels_a']
        self.cycle_oh_labels_b = outputs['cycle_oh_labels_b']
    ...
    def compute_losses(self):
        ...
        *_summ -> Summary variables for above loss functions
        ...
        semantic_consistency_loss_a = \
            self._lambda_sem_a * losses.semantic_consistency_loss(
                real_labels=self.oh_labels_a, generated_labels=self.fake_oh_labels_b
            )
        semantic_consistency_loss_b = \
            self._lambda_sem_b * losses.semantic_consistency_loss(
                real_labels=self.oh_labels_b, generated_labels=self.fake_oh_labels_a
            )

        g_loss_A = \
            cycle_consistency_loss_a + cycle_consistency_loss_b + lsgan_loss_b + semantic_consistency_loss_a
        g_loss_B = \
            cycle_consistency_loss_b + cycle_consistency_loss_a + lsgan_loss_a + semantic_consistency_loss_b
        ...
        g_A_c_vars = [var for var in self.model_vars if 'g_A_c' in var.name]
        g_B_c_vars = [var for var in self.model_vars if 'g_B_c' in var.name]

        enc_A_vars = [var for var in self.model_vars if 'enc_A' in var.name]
        enc_B_vars = [var for var in self.model_vars if 'enc_B' in var.name]

        self.d_A_trainer = optimizer.minimize(d_loss_A, var_list=d_A_vars)
        self.d_B_trainer = optimizer.minimize(d_loss_B, var_list=d_B_vars)
        self.g_A_trainer = optimizer.minimize(g_loss_A, var_list=g_A_vars+g_A_c_vars+enc_A_vars)
        self.g_B_trainer = optimizer.minimize(g_loss_B, var_list=g_B_vars+g_B_c_vars+enc_B_vars)
    ...
    def save_images(self, sess, epoch):
        ...
        names = ['inputA_', 'inputB_', 'inputA_label_', 'inputB_label_', 
                'fakeA_', 'fakeB_', 'fakeA_label_', 'fakeB_label_']
        ...
        with open(os.path.join(
            ...
            for i in range(0, self._num_imgs_to_save):
                ...
                fake_A_temp, fake_B_temp, fake_A_label_temp, fake_B_label_temp = sess.run([
                    self.fake_images_a,
                    self.fake_images_b,
                    self.fake_labels_a,
                    self.fake_labels_b
                    ...
                    self.label_a: inputs['labels_i'],
                    self.label_b: inputs['labels_j']
                })

                tensors = [inputs['images_i'], inputs['images_j'], inputs['labels_i'], inputs['labels_j'],
                           fake_B_temp, fake_A_temp, fake_A_label_temp, fake_B_label_temp]
    ...
    def fake_label_pool(self, num_fakes, fake, fake_pool):
        """
        This function saves the generated image to corresponding
        pool of images.
    ...
    def plot_loss_history(self, g_A_hist, g_B_hist, d_A_hist, d_B_hist):
        ...
        pyplot.legend()
        # save plot to file
        pyplot.savefig('./output/semgan/plot_line_plot_loss.png')
        pyplot.close()
    ...
    def plot_acc_history(self, Acc_real_A_hist, Acc_real_B_hist, Acc_fake_A_hist, Acc_fake_B_hist):
        ...
        pyplot.legend()
        # save plot to file
        pyplot.savefig('./output/semgan/plot_line_plot_accuracy.png')
        pyplot.close()
    ...
    def train(self):
        ...
        self.inputs = data_loader.load_data(
            self._img_dataset_name, self._label_dataset_name,
        ...
        max_images = semgan_datasets.DATASET_TO_SIZES[self._img_dataset_name]
            ...
            for epoch in range(sess.run(self.global_step), self._max_step):
                ...
                if (epoch+1) % self._save_freq == 0:
                    saver.save(sess, os.path.join(
                        self._output_dir, "semgan"), global_step=epoch)
                ...
                for i in range(0, max_images):
                    ...
                    fake_B_label_temp1 = self.fake_label_pool(
                        self.num_fake_inputs, fake_B_label_temp, self.fake_labels_B)
                    ...
                    _, d_B_loss, summary_str = sess.run(
                        ...
                        feed_dict={
                            ...
                            self.fake_pool_B_label: fake_B_label_temp1
                        }
                    )
                    writer.add_summary(summary_str, epoch * max_images + i)

                    # Optimizing the G_B network
                    _, fake_A_temp, fake_A_label_temp, g_B_loss, summary_str = sess.run(
                        [self.g_B_trainer,
                         self.fake_images_a,
                         self.fake_labels_a,
                         self.g_B_loss,
                    ...
                    fake_A_label_temp1 = self.fake_label_pool(
                        self.num_fake_inputs, fake_A_label_temp, self.fake_labels_A)
                    ...
                    _, d_A_loss, summary_str = sess.run(
                        ...
                        feed_dict={
                            ...
                            self.fake_pool_A_label: fake_A_label_temp1
                        }
                    )
                    writer.add_summary(summary_str, epoch * max_images + i)
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            ...
            self._num_imgs_to_save = semgan_datasets.DATASET_TO_SIZES[
                self._img_dataset_name]
...
def main(to_train, log_dir, config_filename, checkpoint_dir):
    ...
    img_dataset_name = str(config['img_dataset_name'])
    label_dataset_name = str(config['label_dataset_name'])
